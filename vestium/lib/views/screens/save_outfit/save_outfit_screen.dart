...
class _SaveOutfitScreenState extends State<SaveOutfitScreen> {
  ...
  void _loadData() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (_createOutfitCubit == null || _placedItems == null) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            context.router.maybePop();
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(AppLocalizations.of(context)!.saveOutfitNoDataError), backgroundColor: Colors.red),
            );
          }
        });
        return;
      ...
      print('✅ Loaded ${_placedItems!.length} items from service');
      print('✅ Captured image available: ${_capturedImagePath != null ? "YES ✅" : "NO ❌"}');
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text(AppLocalizations.of(context)!.saveOutfitNoDataFound, style: const TextStyle(fontSize: 16)),
            ],
          ),
        ),
      );
