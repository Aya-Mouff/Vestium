...
def queue_sync_operation():
    ...
    try:
        ...
        result = sync_service.queue_operation(user_id, data)
...
@sync_bp.route('/queue/pending', methods=['GET'])
@jwt_required()
...
def process_sync():
    ...
    try:
        user_id = int(get_jwt_identity())
        result = sync_service.process_user_queue(user_id)
        return jsonify(result), 200 if result['success'] else 500
...
def pull_data():
    ...
    try:
        user_id = int(get_jwt_identity())
        result = sync_service.get_user_data(user_id)
        return jsonify(result), 200 if result['success'] else 500
...
@sync_bp.route('/admin/run-sync', methods=['POST'])
def run_sync_job():
    """Manual trigger for cron"""
    try:
        # Simple auth
        auth_header = request.headers.get('Authorization')
        if not auth_header or auth_header != 'Bearer YOUR_CRON_SECRET':
            return jsonify({'success': False, 'error': 'Unauthorized'}), 403
        ...
        return jsonify({
            'success': True,
            'message': 'Sync job ran',
            'timestamp': datetime.utcnow().isoformat()
        }), 200
        
    except Exception as e:
        logger.error(f"Run sync error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@sync_bp.route('/admin/cleanup', methods=['POST'])
def cleanup_old():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': f'Cleaned {deleted} old entries'
        }), 200
